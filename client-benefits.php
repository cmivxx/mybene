...
//		Use the IF below to force specific file types,
//		or reverse it to disallow specific file types.
...
<div class="wrap">
	...
	<div style="float:left; width:675px; margin-left:0px; padding:0px;">
		<form method="post" action="<?php echo $_SERVER["../".DIRECTORY_NAME."/REQUEST_URI"]; ?>">
			...
			<table width="650" cellspacing="4">
				...
				<tr>
					<td>
						<label><input name="page_name" type="text" class="validate[required]" style="height:30px; font-size:18px" id="page_name" value="<?php echo $pageInfo->page_name; ?>" size="60" /></label>
					</td>
				</tr>
				<tr>
					<td>  
	...
	<div style="float:left; width:275px; margin-left:50px; padding:5px;">
		...
		<input type="hidden" value="delgroupPages" name="task"/>      
				...
				?>
					<tr class="alternate" id="group-<?php echo $benPage->pid; ?>">
						<th class="check-column" scope="row">
							<?php 
								if( $benPage->welcome == '1' ) { 
					  				echo '';
					  			} else {
					  				echo '<input name="deletePage[]" type="checkbox" id="deletePage[]" value="' . $benPage->pid . '"/>';
					  			}
					  		?>
						</th>
			...
			<center>
				<input type="submit" class="button-secondary delete" name="deleteit" value="Delete Page(s)" id="deleteit" onclick="return confirm('Are you sure you want to delete the page(s) you selected?')"/> &nbsp; <input type="submit" class="button-secondary update" name="updateit" value="Update Page Order" id="updateit"/>
			</center>
		</form>
			...
			<table width="100%" class="widefat">
				...
				?>
					<tr class="alternate" id="group-<?php echo $file->id; ?>">
						...
						<td width="7%"><a onClick="addHtmlLink('<?php echo LINK_UPLOAD_DIRECTORY.$client->path.'/'.$file->file_name; ?>', '<?php echo $file->file_name; ?>')" href="#"><img src="<?php echo IMG_DIRECTORY.'plus.png' ?>" align="absmiddle" title="Add file to page" /></a></td>
					</tr>
				<?php
						}
					}
